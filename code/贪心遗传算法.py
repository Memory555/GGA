...
def print_info(pop, distance_matrix):
    ...
    for dot in range(len(X) - 1):
        plt.annotate(pop[maxfitness][dot], xy=(X[dot], Y[dot]), xytext=(X[dot], Y[dot]), color='red')
    plt.annotate('start', xy=(X[0], Y[0]), xytext=(X[0] + 1, Y[0] + 1), color='red')
    plt.xlabel("横坐标")
    plt.ylabel("纵坐标")
    plt.title("轨迹图")

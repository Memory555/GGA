...
def mutation(DNA, MUTA_RATE):#进行变异
    if np.random.rand() < MUTA_RATE: 				#以MUTA_RATE的概率进行变异
        ...
        # averfitness = np.mean(fitness)
        # fitness1 = (1 / (distance(DNA[mutate_point1])))
        # fitness2 = (1 / (distance(DNA[mutate_point2])))
        # mymaxfitness = max(fitness1, fitness2)
        # # 贪心策略变异算子
        # k_3 = 0.1
        # k_4 = 0.2
        # if mymaxfitness >= averfitness:
        #     MUTA_RATE = (k_3 * (maxfitness - mymaxfitness)) / (maxfitness - averfitness)
        # else:
        #     MUTA_RATE = k_4



def crossmuta(pop, CROSS_RATE):#进行 交叉
    new_pop = []
    for i in range(len(pop)):#遍历种群中的每一个个体，将该个体作为父代
        ...
        if n<CROSS_RATE:#小于交叉概率时发生变异
            ...
            # maxfitness = max(fitness)
            # averfitness = np.mean(fitness)
            # fitness1 = (1 / (distance(list1)))
            # fitness2 = (1 / (distance(list2)))
            # mymaxfitness = max(fitness1,fitness2)
            # # 贪心策略交叉算子
            # k_1 = 0.5
            # k_2 = 0.6
            # if mymaxfitness >= averfitness:
            #     CROSS_RATE = (k_1*(maxfitness-mymaxfitness)) / (maxfitness-averfitness)
            # else:
            #     CROSS_RATE = k_2
...
if __name__ == "__main__":#主循环
    ...
    for i in range(Iterations):  # 迭代N代
        ...
        # if len(best_dis) >= 10 and abs(best_dis[len(best_dis)-1] - best_dis[len(best_dis)-2]) <= 0.000000000001:
        #     break
        # else:
...
#画图
plt.figure()
plt.plot(range(Iter),best_dis)
plt.xlabel("代数(0->1000)")
plt.ylabel("最优值")
plt.title("优化过程")
plt.show()
plt.close()